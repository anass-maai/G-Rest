...
<div class="row an-row-header" >
    <div class="col-xs-3"><?= TAB_LIST_RESTO_RESTORATEUR ?></div>
...
foreach($data["listResto"] as $restaurant) {
    ...
    echo ("<div class='row $rowColore'>
             <div class='col-xs-3'>$restaurant->prenom $restaurant->nom</div>
...
<script>
    $(function() {
        $(document).ready(function(){
            <?php
                 foreach($data["listResto"] as $restaurant) {
       echo ('$("#Bt_showMenu'. $restaurant->idrestaurant .'").click(function(){ $("#menus_restaurant'. $restaurant->idrestaurant .'").load("'.DIR.'restaurateur/menu/List/'. $restaurant->idrestaurant .'#DivlistMenu");  });
                    ');
                 }
            ?>
        });
